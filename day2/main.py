...
def par1():
    cnt_pws = 0
    for l in _list:
        m = re.match(r"(\d+)-(\d+) ([a-zA-Z]): ([a-zA-Z]+)", l)
        if m != None:
            print("m.groups = " + str(m.groups()))
            for i in range(4):
                print("m.group({}) = {}".format(i, m.groups(i)))

            _min, _max, tok, _str = m.groups()
            cnt = _str.count(tok)
            print("count of tok in _str = " + str(cnt))
            if cnt >= int(_min) and cnt <= int(_max):
                print("-------This pw okay!")
                cnt_pws += 1
        else:
            print("no match")
    ...
    print("-----------------------")
    print("Count of passwords = " + str(cnt_pws))
...
def part2():
    ...
    for l in _list:
        ...
        if m != None:
            ...
            _c1, _c2, tok, _str = m.groups()
            match_cnt = 0
            if int(_c1) <= len(_str) and tok == _str[int(_c1)-1]:
                match_cnt += 1
            if int(_c2) <= len(_str) and tok == _str[int(_c2)-1]:
                match_cnt += 1
